...
void initDynamicsBdSpecialPattern(vector<Group> &vecGroupBd, vector<Node> &vecNodeBd, string typePattern)
{
    ...
    if (typePattern == "two_half")
    {
        ...
        vecLineCoordW.push_back(coord_lower_left); vecLineCoordS.push_back(coord_lower_left);
        vecLineCoordN.push_back(coord_upper_right); vecLineCoordE.push_back(coord_upper_right);
...
void writeNodeDynamicsTherapy(vector<Node> &vecNode, double t_now, int seed)
{
    ...
    string nodeFileName;
    stringstream PROC_ID_SS; PROC_ID_SS << PROC_ID;
    stringstream seed_SS; seed_SS << seed;
